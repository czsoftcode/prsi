# Phase 14 — UI výběru motivu (overlay)

**Goal:** Přidat tlačítko "Motiv" vedle indikátoru barvy, které otevře in-app overlay s náhledem každého dostupného motivu (4 karty + rub); klik přepne aktivní motiv, zavře overlay a překreslí stůl bez reloadu stránky.

## Steps
- [done] Helper pro náhledové cesty motivu (assets.ts)
- [done] Export seznamu motivů (theme.ts)
- [done] Overlay výběru motivu (overlay.ts)
- [done] Tlačítko Motiv ve stolu (render.ts)
- [done] Napojení výběru v main.ts
- [done] CSS overlaye motivů (style.css)
- [done] Ověřit zelený build + testy
